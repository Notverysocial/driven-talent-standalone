...
insert into payroll_periods (start_date, end_date, status, invoice_date, approved_by, approved_at, notes) values
  (((current_date - extract(dow from current_date)::int) - interval '2 week')::date,
   ((current_date - extract(dow from current_date)::int) - interval '1 week' - interval '1 day')::date,
   'approved',
   ((current_date - extract(dow from current_date)::int) - interval '1 week' + interval '3 day')::date,
   'Roxanna', now() - interval '8 day',
   'Closed, audited and invoiced.'),
  (((current_date - extract(dow from current_date)::int) - interval '1 week')::date,
   ((current_date - extract(dow from current_date)::int) - interval '1 day')::date,
   'audited',
   ((current_date - extract(dow from current_date)::int) + interval '3 day')::date,
   null, null,
   'Audit complete; awaiting final client approvals.'),
  (((current_date - extract(dow from current_date)::int))::date,
   ((current_date - extract(dow from current_date)::int) + interval '6 day')::date,
   'open', null, null, null,
...
from (values
  ('DT-2026-0461','pacific-vines',
     ...
     ((current_date - extract(dow from current_date)::int) - interval '1 week' - interval '1 day')::date,
     ((current_date - extract(dow from current_date)::int) - interval '1 week' + interval '3 day')::date,
     ((current_date - extract(dow from current_date)::int) - interval '1 week' + interval '3 day' + interval '30 day')::date,
